...
class Ftl(object):
    ...
    def write_ext(self, extent):
        ...
        if self.written_bytes > self.pre_written_bytes + self.display_interval:
            print 'Written (MB)', self.written_bytes / MB, 'writing', round(float(req_size) / MB, 2)
            sys.stdout.flush()
    ...
    def read_ext(self, extent):
        ...
        if self.read_bytes > self.pre_read_bytes + self.display_interval:
            print 'Read (MB)', self.read_bytes / MB, 'reading', round(float(req_size) / MB, 2)
            sys.stdout.flush()
    ...
    def discard_ext(self, extent):
        ...
        if self.discarded_bytes > self.pre_discarded_bytes + self.display_interval:
            print 'Discarded (MB)', self.discarded_bytes / MB, 'discarding', round(float(req_size) / MB, 2)
            sys.stdout.flush()
