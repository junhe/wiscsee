...
class TestFTLwithDFTL(unittest.TestCase):
    ...
    def setup_ftl(self):
        ...
        self.conf.max_cmt_bytes = self.conf.n_mapping_entries_per_page * 8 # 8 bytes (64bits) needed in mem
...
class Test_translation_cache(unittest.TestCase):
    ...
    def setup_ftl(self):
        ...
        devsize_mb = 8
        self.conf.max_cmt_bytes = int(self.conf.n_mapping_entries_per_page * 8) # 8 bytes (64bits) needed in mem
        self.conf.set_flash_num_blocks_by_bytes(int(devsize_mb * 2**20 * 1.3))
        print "Current n_blocks_per_plane",\
            self.conf['flash_config']['n_blocks_per_plane']
