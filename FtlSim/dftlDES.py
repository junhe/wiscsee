...
class MappingManager(object):
    ...
    def update_entry(self, lpn, new_ppn, tag = "NA"):
        ...
        yield g_env.process(
                self.update_translation_page_on_flash(
                    m_vpn, new_mappings, tag))
    ...
    def evict_cache_entry(self):
        ...
        if vic_entrydata.dirty == True:
            ...
            yield self.batch_write_back(m_vpn)
    ...
    def batch_write_back(self, m_vpn):
        ...
        yield g_env.process(
                self.update_translation_page_on_flash(
                    m_vpn, new_mappings, TRANS_CACHE))
    ...
    def update_translation_page_on_flash(self, m_vpn, new_mappings, tag):
        ...
        if len(new_mappings) < self.conf.dftl_n_mapping_entries_per_page():
            ...
            yield g_env.process(self.flash.page_read(old_m_ppn, tag))
        else:
        ...
        # update flash
        yield g_env.process(self.flash.page_write(new_m_ppn, tag))
...
class GarbageCollector(object):
    ...
    def try_gc(self):
        ...
        while self.decider.need_cleaning():
            ...
            if victim_type == DATA_BLOCK:
                yield g_env.process(self.clean_data_block(victim_block))
            elif victim_type == TRANS_BLOCK:
                yield self.clean_trans_block(victim_block)
            blk_cnt += 1
    ...
    def clean_data_block(self, flash_block):
        ...
        for ppn in range(start, end):
            if self.oob.states.is_page_valid(ppn):
                change = yield g_env.process(
                        self.move_data_page_to_new_location_2(ppn))
                changes.append(change)

        # change the mappings
        yield self.update_mapping_in_batch(changes)
    ...
    def clean_trans_block(self, flash_block):
        yield g_env.process( self.move_valid_pages(flash_block,
    ...
    def move_valid_pages(self, flash_block, mover_func):
        ...
        for ppn in range(start, end):
            if self.oob.states.is_page_valid(ppn):
                yield g_env.process(mover_func(ppn))
    ...
    def move_data_page_to_new_location(self, ppn):
        ...
        ****** Currently This Function is not in Use ******
        ...
        yield g_env.process(
                self.flash.page_write(
                    new_ppn, DATA_CLEANING, data = pagedata))
    ...
    def move_data_page_to_new_location_2(self, ppn):
        ...
        g_env.exit( {'lpn':lpn, 'old_ppn':old_ppn, 'new_ppn':new_ppn} )
    ...
    def erase_block(self, blocknum, tag):
        ...
        yield g_env.process(self.flash.block_erase(blocknum, tag))
...
class Dftl(ftlbuilder.FtlBuilder):
    ...
    def __init__(self, confobj, recorderobj, flashobj, simpy_env):
        ...
        flashobj = flash.FlashDES(recorderobj, confobj,
                self.global_helper, simpy_env)
        self.flash = flashobj
        self.env = simpy_env
        global g_env
        g_env = simpy_env
    ...
    def lba_read(self, lpn, pid = None):
        ...
        ppn = yield g_env.process(
            self.mapping_manager.lpn_to_ppn(lpn) )
        data = yield g_env.process(self.flash.page_read(ppn, DATA_USER))
        ...
        yield g_env.process( self.garbage_collector.try_gc() )
    ...
    def lba_write(self, lpn, data = None, pid = None):
        ...
        old_ppn = yield g_env.process(self.mapping_manager.lpn_to_ppn(lpn))
    ...
    def lba_discard(self, lpn, pid = None):
        ...
        ppn = yield g_env.process(self.mapping_manager.lpn_to_ppn(lpn))
        if ppn == UNINITIATED:
            return
